...
(./or_logic.JPG)
>
> However, we can continue with the same query, but select which row we want to actually be display using ```' or 1=1 limit 1 offset 9#```.  This returns the uniform seen at ```http://referee.polictf.it/uniform/10```, so we must be off by one. Changing the query yields:
>
![Image of flag]
(./flag.JPG)
